...
// ---Components
import DaySelector from 'Comp/UtilityWeek/DaySelector';
...
function UtilityWeek() {
  ...
  function onDateChange(data) {
    const fixedDate = dateFormToWeekStartEnd(data);
  ...
  function dataToGraphDays(orders) {
    ...
    if (newOrders) {
      newOrders = orders.map(order => ({
        ...order,
        date: dateMongoToWeekDays(order.date)
      }));
    }

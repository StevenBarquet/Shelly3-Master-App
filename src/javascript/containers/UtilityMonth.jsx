...
// ---Components
import DaySelector from 'Comp/UtilityMonth/DaySelector';
...
function UtilityMonth() {
  ...
  function onDateChange(data) {
    const fixedDate = dateFormToMonthStartEnd(data);
  ...
  function dataToGraphDays(orders) {
    ...
    if (newOrders) {
      newOrders = orders.map(order => ({
        ...order,
        date: dateMongoToDays(order.date)
      }));
    }
